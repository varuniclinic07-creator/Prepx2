...
// GET: check eligibility + existing contract
export async function GET() {
  ...
  if (!prediction || prediction.confidence_pct < 60) {
    return NextResponse.json({ eligible: false, reason: 'Need prediction confidence ≥ 60%' });
  }
  const rankOrder = ['ASO', 'Deputy Collector', 'Collector', 'Secretary', 'Cabinet Secretary'];
  if (rankOrder.indexOf(rank?.current_rank || '') < rankOrder.indexOf('Collector')) {
    return NextResponse.json({ eligible: false, reason: 'Need rank Collector or higher' });
  }
