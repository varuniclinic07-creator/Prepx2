  ...
  return (
    <div className="max-w-sm mx-auto mt-20 space-y-6">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-slate-100">Join prepx</h1>
        <p className="text-slate-400 text-sm mt-1">Start your UPSC journey today</p>
      </div>

      {message && <div className={`text-sm p-3 rounded-lg ${message.includes('Check') ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>{message}</div>}
      ...
      <form onSubmit={handleSignup} className="space-y-4">
        ...
        <button type="submit" disabled={loading} className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-slate-950 font-bold rounded-xl transition">{loading ? 'Creating account...' : 'Create Account'}</button>
      </form>
      ...
      <p className="text-center text-sm text-slate-500">Already have an account? <Link href="/login" className="text-emerald-400 hover:underline">Sign in</Link></p>
    </div>
  );
}
