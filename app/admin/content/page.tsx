...
function ContentPageInner() {
  ...
  useEffect(() => {
    setSubject(preselected);
    loadCount(preselected);
  }, [preselected]);
  ...
  const runBatch = async () => {
    setLoading(true);
    setResult(`Batch generating for ${subject}...`);
    try {
      const subj = ALL_SUBJECTS.find(s => s.id === subject);
      if (!subj) throw new Error('Unknown subject');
      ...
      setResult(`Batch complete for ${subject}: ${seeded} topics seeded. Total: ${topicCount + seeded}`);
    } catch (e: any) {
      setResult(`Error: ${e.message}`);
    }
    setLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-slate-400">Auto-generate topics via the multi-provider AI router.</p>

      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
        <label className="text-sm text-slate-400">Subject</label>
        <select
          value={subject}
          onChange={e => { setSubject(e.target.value); loadCount(e.target.value); }}
        ...
        <div className="flex items-center justify-between text-sm">
          <span className="text-slate-400">Current topics: <strong className="text-slate-200">{topicCount}</strong></span>
          <span className="text-slate-400">Target: <strong className="text-slate-200">{subjInfo?.totalTopics ?? 0}</strong></span>
        </div>

        <div className="w-full bg-slate-800 rounded-full h-2">
          <div
            className="h-2 rounded-full bg-emerald-500 transition-all"
            style={{ width: `${Math.min(100, (topicCount / Math.max(1, subjInfo?.totalTopics ?? 1)) * 100)}%` }}
          />
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            {loading ? 'Generating...' : 'Generate 1 Topic'}
          </button>
          <button
            onClick={runBatch}
            disabled={loading}
          ...
          >
            {loading ? 'Running...' : 'Generate 3 Topics'}
          </button>
        </div>
      </div>
