...
**Honest gaps (Sprint 5-part-2 backlog, not blocking commit):**
- AIStrategist text is **static placeholder copy** (Chanakya prompt). Wiring it to a real `/api/strategist/diagnose` endpoint that calls `aiChat()` against the user's last-30-day quiz/mains history is the next sub-batch.
- `/api/podcast/generate-user-episode` returns audio as a `data:audio/mp3;base64,...` URL — works for ≤4000-char scripts but not durable. Migrate to Supabase Storage `podcasts` bucket with signed URL refresh (mirroring `lib/video/storage.ts`) in part 2.
- No public `/podcast` page yet — episode is generated by the API but the user-facing player route (`app/podcast/page.tsx` / `app/podcast/[date]/page.tsx`) needs wiring before this feature is end-to-end visible from the dashboard.
- Dashboard MasteryTrajectory uses `quiz_attempts.completed_at` as the X-axis bucket. If a user has no quiz history the chart renders 7 zero-bars rather than showing a friendly "Take your first quiz" empty state.
